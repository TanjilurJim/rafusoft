...
@section('content')
    ...
    <section class="p-3">
            <div class="bg-[#343A40] p-3 rounded text-white flex items-center justify-between">
                <h1 class="text-[20px]">Dir Pages</h1>
                <a href="/dashboard/dir/create" class="btn btn-orange">Create New Page</a>
            </div>
            <div class="mt-3">
                    ...
                    <tbody>
                        @foreach ($dir as $item)
                            <tr>
                                ...
                                <td>{{ $item->slug }}</td>
                                <td>
                                    <a target="_blank" href="/dashboard/preview/dir/{{ $item->slug }}"
                                        class="btn  btn-secondary btn-sm">Preview</a>
                                    <a target="_blank" href="/dir/{{ $item->slug }}"
                                        class="btn btn-success btn-sm">View</a>
                                    <button onclick="handleCopy('<?php echo $item->slug; ?>')"
                                        class="btn btn-secondary btn-sm">Copy</button>
                                    <a href="/dashboard/dir/edit/{{ $item->id }}"
                                        class="btn  bg-orange-500 text-white font-medium btn-sm">Edit</a>
                                    <a href="/dashboard/dir/faq/{{ $item->id }}"
                                        class="btn  bg-blue-500 text-white font-medium btn-sm">Faq</a>
                                    <a href="/dashboard/dir/pop-up/{{ $item->id }}"
                                        class="btn  bg-teal-500 text-white font-medium btn-sm">Pop Up</a>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form method="POST" action="/dashboard/dir-delete/{{ $item->id }}">
                                                @csrf @method('DELETE')
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="px-3 pt-2">
                                            <form method="POST" id="updateStatus{{ $item->id }}"
                                                action="/dashboard/dir/updated-status/{{ $item->id }}">
                                                @csrf
                                                ...
                                                <div class="flex gap-3 items-baseline">
                                                    <input name="enquire" type="checkbox" @checked($item->enquire) id="enquire{{ $item->id }}" value="1">
                                                    <label for="enquire{{ $item->id }}">Enquire</label>
                                                </div>
                                                <div class="flex gap-3 items-baseline">
                                                    <input name="menu" type="checkbox" @checked($item->menu) id="menu{{ $item->id }}" value="1">
                                                    <label for="menu{{ $item->id }}">Menu</label>
                                                </div>
                                                <div class="modal-footer">
        ...
        <script>
            const handleSubmitStatus = (id) => {
                const form = document.getElementById('updateStatus' + id);
                form.submit();
            }
