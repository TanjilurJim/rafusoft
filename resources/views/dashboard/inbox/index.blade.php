...
@section('content')
    <section class="p-3 relative">
        <div class="bg-[#343A40] rounded text-white flex justify-between p-3">
            <h1 class="text-[20px]">Inbox Contact (Inbox {{ count($messages) }} | Unread {{ $unread }} | Read  {{ $read }}) </h1>
                            ...
                            <td class="flex gap-5 items-center justify-center ">
                                <div class="hoverButtonWrapper relative">
                                    <div class="actionButtonDiv absolute top-[-29px] bg-black right-0 px-2 py-2 rounded-3xl shadow-sm">
                                        <a href="/dasboard/inbox/contact/great/{{$item->id}}/1"><i class="fa-solid  text-green-500 fa-circle"></i></a>
                                        <a href="/dasboard/inbox/contact/great/{{$item->id}}/2"><i class="fa-solid  text-yellow-500 fa-circle"></i></a>
                                        <a href="/dasboard/inbox/contact/great/{{$item->id}}/3"><i class="fa-solid  text-red-500 fa-circle"></i></a>
                                    </div>
                                ...
                                <a title="view" href="/dasboard/inbox/contact/view/{{ $item->id }}?no={{$index-1}}"
                                    class="hover:text-blue-500 btn"><i class="fa-regular fa-eye"></i></a>

                                <form id="updatedfrom{{ $item->id }}"
                                    action="/dasboard/inbox/contact/status/{{ $item->id }}" method="POST"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <form method="POST" action="/dasboard/inbox/contact/delete/{{ $item->id }}">
                                            @csrf @method('DELETE')
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form action="/dasboard/inbox/contact/delete-groupby" method="POST" class="modal-content">
                    @method('DELETE')
                    @csrf
                <div class="modal-header">
        ...
        <script>
            ...
            const viewMessage = (id, index) => {
                window.location.href = `/dasboard/inbox/contact/view/${id}?no=${index}`
            }
