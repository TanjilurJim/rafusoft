...
@section('content')
    <section class="p-3">
        ...
        <div class="bg-[#343A40] p-3 rounded text-white">
            <h4 class="text-[20px]">Faq <strong class="text-orange-600">({{ $ref->slug }})</strong></h4>
        </div>

        
        <form method="POST" class="mt-5" action="/dashboard/ref/faq/toggle/{{ $ref->id }}" id="faqForm">
            @csrf

            <div class="custom-control custom-switch  custom-switch-on-orange border-0">
                <input  onchange="handleFormSubmit()" {{ $ref->faq == 'off' ? '' : 'checked' }}  type="checkbox"
                    class="custom-control-input" id="customSwitch1{{ $ref->id }}">
                <label class="custom-control-label" for="customSwitch1{{ $ref->id }}">Active</label>
            </div>
        </form>
        ...
        <div class="accordion mt-5">
            ...
            <div class="flex gap-4">
                <a href="/dashboard/ref/faq/edit/{{$item->id}}/{{$ref->id}}" class="btn btn-orange">Edit</a>
                ...
                <div class="modal-dialog">
                  <div class="modal-content">
                    ...
                    <div class="modal-footer">
                      ...
                      <form method="POST" action="/dashboard/ref/faq-delete/{{$item->id}}">
                        @csrf
                        @method("DELETE")
