...
@section('content')
    <section class="p-3">
        ...
        <div class="bg-[#343A40] p-3 rounded text-white flex justify-between items-center mb-3">
            <h1 class="text-[20px]">Terms & Conditions</h1>
            <a href="/dashboard/product/setting" class="btn btn-orange">Back</a>
        </div>

        <section>
            <form action="/dashboard/product/setting/terms" method="POST">
              @csrf
              <textarea name="terms" class="tinymce " cols="30" rows="10">{!!$terms ?  $terms->terms : "" !!}</textarea>
