...
class PublisherImageController extends Controller
{
    public function index(Request $request)
    {

        $mypath = auth()->user()->user_id;
        $destinationPath = public_path("filemanager/publisher_image/$mypath");
        ...
        if(auth()->user()->role == "superadmin"){
            $destinationPath = public_path('publisher_image');
        }



        $image = File::allFiles($destinationPath);
        return view("dashboard.file.publisher.index" ,["icons" => $image]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (File::exists($destinationPath . '/' . $filenameWithoutExtension.".jpg")) {
        ...
        imagejpeg($image, "$destinationPath/".$filenameWithoutExtension.".jpg", 100);
        imagedestroy($image);
    ...
    public function rename(Request $request)
    {
        ...
        if ($filename && $newname) {
            ...
            foreach ($icons as $file) {
                if ($file->getFilename() === $filename . ".jpg") {
                    $oldPath = $file->getPathname();
                    $newPath = $file->getPath() . DIRECTORY_SEPARATOR . $newname . ".jpg";
