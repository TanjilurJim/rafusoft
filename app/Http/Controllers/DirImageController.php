...
class DirImageController extends Controller
{
    public function index(Request $request){


        $mypath = auth()->user()->user_id;
        $destinationPath = public_path("filemanager/dir_image/$mypath");
        if (!File::exists($destinationPath)) {
            File::makeDirectory($destinationPath, 0755, true, true);
        }
        if(auth()->user()->role == "superadmin"){
            $destinationPath = public_path('dir_image');
        }



        $image = File::allFiles($destinationPath);
        return view("dashboard.file.dir.index", ['icons' => $image]);    
    }
